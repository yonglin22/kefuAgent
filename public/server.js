...
function getLastOrderId(session, text) {
  const directMatch = String(text || '').match(/\b\d{6,20}\b/);
  if (directMatch) return directMatch[0];
  if (session && session.lastOrderId) return session.lastOrderId;
  for (let i = session.messages.length - 1; i >= 0; i--) {
    const msg = session.messages[i];
    const match = String(msg && msg.content || '').match(/\b\d{6,20}\b/);
    if (match) return match[0];
  }
  return '';
}

function buildFallbackResponse(normalizedMessage, session = null) {
  ...
  if (lastOrderId) {
    if (/退款|退货|退款进度|原路退回/.test(q) || pendingIntent === 'refund') {
      const refund = queryRefund(lastOrderId);
      if (refund) {
        return {
          type: 'refund',
          content: `已找到订单 ${lastOrderId}，退款当前为【${refund.status}】，预计 ${refund.estimatedArrival} 到账。`,
          displayContent: `已找到订单 ${lastOrderId}，退款当前为【${refund.status}】，预计 ${refund.estimatedArrival} 到账。`,
          extra: { order_id: lastOrderId, refund }
        };
      }
      return {
        type: 'refund',
        content: `我查到了订单号 ${lastOrderId}，但暂时没有找到对应退款记录。请再确认一下订单号。`,
        displayContent: `我查到了订单号 ${lastOrderId}，但暂时没有找到对应退款记录。请再确认一下订单号。`,
        extra: { order_id: lastOrderId }
      };
    }

    if (/物流信息不更新|物流不更新|物流延迟|物流信息卡住|物流没更新|物流|快递|轨迹|物流信息|到哪/.test(q) || pendingIntent === 'logistics' || pendingIntent === 'logisticsDelay') {
      const logistics = queryLogistics(lastOrderId);
      if (logistics) {
        const lastRecord = logistics.records && logistics.records[logistics.records.length - 1];
        return {
          type: 'logistics',
          content: `已找到订单 ${lastOrderId}，物流当前为【${lastRecord ? lastRecord.desc : logistics.company}】，预计 ${logistics.estimatedDelivery} 送达。`,
          displayContent: `已找到订单 ${lastOrderId}，物流当前为【${lastRecord ? lastRecord.desc : logistics.company}】，预计 ${logistics.estimatedDelivery} 送达。`,
          extra: { order_id: lastOrderId, logistics }
        };
      }
      return {
        type: 'logistics',
        content: `我查到了订单号 ${lastOrderId}，但暂时没有找到物流记录。请再确认一下订单号。`,
        displayContent: `我查到了订单号 ${lastOrderId}，但暂时没有找到物流记录。请再确认一下订单号。`,
        extra: { order_id: lastOrderId }
      };
    }

    if (/订单|下单|发货/.test(q) || pendingIntent === 'order') {
      const order = queryOrder(lastOrderId);
      if (order) {
        return {
          type: 'order',
          content: `已找到订单 ${lastOrderId}，当前状态【${order.status}】，商品是【${order.product}】。`,
          displayContent: `已找到订单 ${lastOrderId}，当前状态【${order.status}】，商品是【${order.product}】。`,
          extra: { order_id: lastOrderId, order }
        };
      }
      return {
        type: 'order',
        content: `我查到了订单号 ${lastOrderId}，但暂时没有找到对应订单记录。请再确认一下订单号。`,
        displayContent: `我查到了订单号 ${lastOrderId}，但暂时没有找到对应订单记录。请再确认一下订单号。`,
        extra: { order_id: lastOrderId }
      };
    }
  }
...
app.post('/api/chat', async (req, res) => {
  ...
  const lastOrderId = getLastOrderId(session, normalizedMessage);
  ...
  try {
    ...
    if (!CONFIG.deepSeek.apiKey) {
      const fallback = buildFallbackResponse(normalizedMessage, session);
      if (lastOrderId) session.lastOrderId = lastOrderId;
      return res.json({
    ...
    if (choice.message.tool_calls && choice.message.tool_calls.length > 0) {
      ...
      if (toolName === 'query_order') {
        ...
        const orderId = args.order_id || lastOrderId;
        toolResult = { result: queryOrder(orderId), order_id: orderId };
        if (orderId) session.lastOrderId = orderId;
      } else if (toolName === 'query_logistics') {
        const args = JSON.parse(toolCall.function.arguments);
        const orderId = args.order_id || lastOrderId;
        toolResult = { result: queryLogistics(orderId), order_id: orderId };
        if (orderId) session.lastOrderId = orderId;
      } else if (toolName === 'query_refund') {
        const args = JSON.parse(toolCall.function.arguments);
        const orderId = args.order_id || lastOrderId;
        toolResult = { result: queryRefund(orderId), order_id: orderId };
        if (orderId) session.lastOrderId = orderId;
